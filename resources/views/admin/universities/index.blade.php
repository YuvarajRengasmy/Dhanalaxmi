...
<div class="card ">
    <div class="card-body">
        ...
        <div class=" card-header-actions d-flex justify-content-end mb-2 ">
            <a class="btn btn-success btn-sm" href="{{ route('universities.create') }}">
                ...
                <tbody>
                    @forelse ($universities as $index => $university)
                        <tr>
                            ...
                            <td class="text-capitalize">{{ $university->created_at->format('d-m-Y') }}</td>
                            <td class="text-capitalize">{{ $university->name }}</td>
                            <td class="text-capitalize"><img src="{{ asset('storage/' . $university->image) }}" alt="Logo" width="100" height="100"></td>
                            <td class="text-capitalize">{{ $university->location }}</td>
                           
                            <td>
                                <form action="{{ route('universities.destroy', $university->id) }}" method="POST">
                                    <a class="btn btn-info btn-sm" href="{{ route('universities.show', $university->id) }}">
                                        <i class="fa fa-eye"></i> 
                                    </a>
                                    <a class="btn btn-primary btn-sm" href="{{ route('universities.edit', $university->id) }}">
                                        <i class="fa fa-pen"></i> 
                                    </a>
                                    @csrf
                                    @method('DELETE')
